...
class Abstract_Model(object):
	...
	def __init__(self):
		...
		#self.weights = TT.lvector('weights')
			...
			if it % hparams.valid_scores_every == 0 and scorer is not None:
				if valid_triples is not None:
					...
					if scorer.compute_ranking_scores:
						...
						if best_valid_mr < metrics[self.name][2]:
							logger.info("Validation filtered MR increased, stopping here.")
							break
						else:
							best_valid_mr = metrics[self.name][2]
					else:
...
class TransE_L2_Model(Abstract_Model):
	...
	def get_init_params(self):
		...
		params = { 'e' : L2_proj(randn(max(self.n,self.l),self.k)), #np.random.uniform(-6.0/np.sqrt(self.k), 6.0/np.sqrt(self.k), (max(self.n,self.l),self.k)).astype('f')
				   #'b' : L2_proj(randn(self.m,self.k)), #L2_proj(np.random.uniform(-6.0/np.sqrt(self.k), 6.0/np.sqrt(self.k), (self.m,self.k))).astype('f')
				   'r': L2_proj(randn(self.m, self.k)),
				   #'w' : L2_proj(randn(self.m, self.k)),
				   'w1': L2_proj(randn(max(self.n, self.l), self.k)),
				   'w2': L2_proj(randn(self.m,self.k))
				   }
		return params
	...
	def define_loss(self):
		...
		self.pred_func = - TT.sqrt(TT.sum(TT.sqr((self.e[self.tubes,:] - self.e[self.rows,:]) * self.w[self.cols,:]  + self.e[self.cols,:]),1))

		self.loss = TT.maximum( 0, self.margin + TT.sqrt(TT.sum(TT.sqr((self.e[self.tubes[:self.batch_size],:]  - self.e[self.rows[:self.batch_size],:]) * self.w[self.cols[:self.batch_size],:] + self.b[self.cols[:self.batch_size],:]),1)) \
			- (1.0/self.neg_ratio) * TT.sum(TT.sqrt(TT.sum(TT.sqr((self.e[self.tubes[self.batch_size:],:] - self.e[self.rows[self.batch_size:],:]) * self.w[self.rows[self.batch_size:],:] + self.b[self.cols[self.batch_size:],:]),1)).reshape((int(self.batch_size),int(self.neg_ratio))),1) ).mean()
		...
		self.regul_func = 0.004 * TT.sqr(self.w[self.cols,:]).mean() \
						+ 0.001 * TT.sqr(self.b[self.cols,:]).mean()
			#Entities embeddings are force to have unit norm, as stated in TransE paper
...
class wTransE_2L_Model(TransE_L2_Model):
	...
	def define_loss(self):
		...
		pre1 = self.miuA * TT.abs_((self.e[self.tubes,:] - self.e[self.rows,:]) * self.w1[self.rows,:] - self.r[self.cols,:] * self.w2[self.cols,:])
		pre2 = self.miuB * TT.abs_(self.e[self.rows,:] + self.r[self.cols,:] - self.e[self.tubes,:])
		pre_l1 = 0.5 * (pre1 + pre2)
		self.pred_func = - TT.sum(pre_l1,1)

		loss1 = self.miuA * TT.abs_((self.e[self.tubes[:self.batch_size],:] - self.e[self.rows[:self.batch_size],:]) * self.w1[self.rows[:self.batch_size],:] - self.r[self.cols[:self.batch_size],:] * self.w2[self.cols[:self.batch_size],:])
		loss2 = self.miuB * TT.abs_(self.e[self.rows[:self.batch_size],:] + self.r[self.cols[:self.batch_size],:] - self.e[self.tubes[:self.batch_size],:])
		loss_l1 = 0.5 * (loss1 + loss2)
		loss1_n = self.miuA * TT.abs_((self.e[self.tubes[self.batch_size:],:] - self.e[self.rows[self.batch_size:],:]) * self.w1[self.rows[self.batch_size:],:] - self.r[self.cols[self.batch_size:],:] * self.w2[self.cols[self.batch_size:],:])
		loss2_n = self.miuB * TT.abs_(self.e[self.rows[self.batch_size:],:] + self.r[self.cols[self.batch_size:],:] - self.e[self.tubes[self.batch_size:],:])
		nloss_l1 = 0.5 * (loss1_n + loss2_n)
		self.loss = TT.maximum( 0, self.margin + TT.sum(loss_l1,1) \
			- (1.0/self.neg_ratio) * TT.sum(TT.sum(nloss_l1,1).reshape((int(self.batch_size),int(self.neg_ratio))),1) ).mean() \
...
class wTransE_1L_Model(Abstract_Model):
	...
	def get_init_params(self):
		...
		params = {'e': L2_proj(randn(max(self.n, self.l), self.k)),
				  #'e': np.random.uniform(-6.0/np.sqrt(self.k), 6.0/np.sqrt(self.k), (max(self.n,self.l),self.k)).astype('f'),
				  'r': L2_proj(randn(self.m, self.k)),
				  'w': L2_proj(randn(max(self.n, self.l), self.k)),
				  #'r': L2_proj(np.random.uniform(-6.0/np.sqrt(self.k), 6.0/np.sqrt(self.k), (self.m,self.k))).astype('f'),
				  'b': L2_proj(randn(self.m, self.k))
				  }
		return params
	...
	def define_loss(self):
		...
		predSVM1 = self.miuA * TT.abs_((self.e[self.tubes, :] - self.e[self.rows, :]) * self.w[self.cols, :] - self.b[self.cols, :])
		predSVM2 = self.miuB * TT.abs_(self.e[self.rows, :] + self.r[self.cols,:] - self.e[self.tubes, :])
		predSVM = 0.5 * (predSVM1 + predSVM2)
		self.pred_func = - TT.sum(predSVM,1)

		lossSVM1 = self.miuA * TT.abs_((self.e[self.tubes[:self.batch_size], :]  - self.e[self.rows[:self.batch_size],:] ) * self.w[self.cols[:self.batch_size], :] - self.b[self.cols[:self.batch_size],:])
		lossSVM2 = self.miuB * TT.abs_(self.e[self.rows[:self.batch_size],:] + self.r[self.cols[:self.batch_size],:] - self.e[self.tubes[:self.batch_size], :])
		lossSVM = 0.5 * (lossSVM1 + lossSVM2)

		nlossSVM1 = self.miuA * TT.abs_((self.e[self.tubes[self.batch_size:], :] - self.e[self.rows[self.batch_size:],:]) * self.w[self.cols[self.batch_size:], :] - self.b[self.cols[self.batch_size:],:])
		nlossSVM2 = self.miuB * TT.abs_(self.e[self.rows[self.batch_size:],:] + self.r[self.cols[self.batch_size:],:] - self.e[self.tubes[self.batch_size:], :])
		nlossSVM = 0.5 * (nlossSVM1 + nlossSVM2)
		self.loss = TT.maximum(0, self.margin + TT.sum(lossSVM,1) - (1.0 / self.neg_ratio) * TT.sum(TT.sum(nlossSVM,1).reshape((int(self.batch_size), int(self.neg_ratio))), 1)).mean()
		

		self.regul_func = self.lambda_A * TT.sqr(self.w[self.cols, :]).mean() \
						+ self.lambda_B * TT.sqr(self.b[self.cols, :]).mean()
					# Entities embeddings are force to have unit norm, as stated in TransE paper
		#self.regul_func=0

...
def Normalize(p,k):
    for a in range(k):
        nrm = np.linalg.norm(p[:k])
        if nrm != 0:
            # round values for faster computation of AUC-PR
            p[a] = np.round_(p[a] / nrm, decimals=8)
    return p
...
class Parameters(object):
	...
	def __init__(self, lmbda = 0.1, embedding_size = 100, batch_size =100, max_iter = 1000, learning_rate = 0.1, neg_ratio = 0, contiguous_sampling = False,
			valid_scores_every = 50, learning_rate_policy = 'adagrad',lambda_svm =1):
		...
		self.lambda_svm = lambda_svm                    # Parameter of regular item weight.
